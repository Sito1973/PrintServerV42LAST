...
// Función para broadcastear estadísticas de usuarios (throttled)
let lastBroadcast = 0;
function broadcastUserStats() {
  if (!socketServer) return;

  // Throttle broadcasts to once per minute max
  const now = Date.now();
  if (now - lastBroadcast < 60000) return;
  lastBroadcast = now;
  ...
  const stats = {
    ...
    users: Array.from(activeUsers.values()).map(user => ({
      ...
      userId: user.userId,
      duration: Math.floor((Date.now() - user.joinTime.getTime()) / 1000)
    })),
...
export function setupWebSocket(httpServer: HTTPServer) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('authenticate', async (data) => {
      ...
      console.log(`🔐 [WS] Autenticando socket ${socket.id}`);
      ...
      try {
        ...
        if (user) {
          ...
          console.log(`✅ [WS] Autenticado: ${user.username} (${socket.id})`);

          // Verificación básica de salas
          setTimeout(() => {
            const userRoom = io.sockets.adapter.rooms.get(`user-${user.id}`);
            const printJobsRoom = io.sockets.adapter.rooms.get('print-jobs');

            if (!userRoom || userRoom.size === 0) {
              console.log(`🚨 [WS] Reintentando unirse a salas`);
              socket.join(`user-${user.id}`);
              socket.join('print-jobs');
            }
          }, 100);
          ...
          if (pendingEvents.length > 0) {
            console.log(`📤 [WS] Enviando ${pendingEvents.length} eventos pendientes`);
          ...
          // Ping de verificación silencioso
          setTimeout(() => {
    ...
    socket.on('disconnect', (reason) => {
      const userId = connectedClients.get(socket.id);
      console.log(`🚨 [WS] Desconexión: ${socket.id} (${reason})`);
  ...
  // Monitoreo simplificado cada 5 minutos
  setInterval(() => {
    ...
    console.log(`💚 [WS] Monitor: ${allSockets.size} sockets, ${clientCount} clientes, ${activeUsers.size} usuarios activos`);

    // Solo broadcastear stats si hay usuarios activos
